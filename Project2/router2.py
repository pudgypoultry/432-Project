...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    socketTo3 = create_socket("127.0.0.1", 8003)
    ...
    while True:
        ...
        packet = receive_packet(connection, max_buffer_size, 2)
        sendTo = -1
        ...
        if sendTo == "8003" and new_ttl != 0:
            print("sending packet", new_packet, "to Router 3")
            socketTo3.send(new_packet.encode())
            f = open("output/sent_by_router_2.txt", "a")
            f.write(new_packet + "\n")
            f.close()
            ...
            f = open("output/out_router_2.txt", "a")
            f.write(payload + "\n")
            f.close()
            print("OUT:", payload)
            ...
            f = open("output/discarded_by_router_2.txt", "a")
            f.write(new_packet)
            f.close()
