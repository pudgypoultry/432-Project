...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        packet = receive_packet(con, max_buffer_size, 6)
        sendTo = -1
        ...
        if sendTo == default_gateway_port and new_ttl != 0:
            f = open("output/out_router_6.txt", "a")
            f.write(payload + "\n")
            f.close()
            print("OUT:", payload)
            ...
            f = open("output/discarded_by_router_6.txt", "a")
            f.write(new_packet)
            f.close()
